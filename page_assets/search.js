function truncate_string(input_string) {
    return input_string.toLowerCase().replace(/[-_]/, " ").trim().replaceAll(/[^a-zA-Z0-9\s]/g, "");
}
...
function update_search() {
    ...
    for (const search_item of SEARCH_INDEX) {
        ...
        if (index_name_truncated.includes(search_text_truncated)) {
            score += 1; // partial any position match
        }
        const partial_any_word_start = new RegExp(`(^| )${RegExp.escape(search_text_truncated)}`);
        if (index_name_truncated.search(partial_any_word_start) !== -1) {
            score += 10; // partial any word start match
        }
        const partial_start = new RegExp(`^${RegExp.escape(search_text_truncated)}`);
        if (index_name_truncated.search(partial_start) !== -1) {
            score += 100; // partial start match
        }
        const full_any_word = new RegExp(`(^| )${RegExp.escape(search_text_truncated)}( |$)`);
        if (index_name_truncated.search(full_any_word) !== -1) {
            score += 1000; // full any word match
        }
        const full_start_word = new RegExp(`^${RegExp.escape(search_text_truncated)}( |$)`)
        if (index_name_truncated.search(full_start_word) !== -1) {
